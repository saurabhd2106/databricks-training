"""Gold-layer aggregations for actuarial reporting."""
...
def build_gold_tables(spark: SparkSession, catalog: str, schema: str) -> dict[str, DataFrame]:
    """Read silver tables, build gold aggregations, and persist them."""
    claims = spark.table(f"{catalog}.{schema}.silver_claims_bordereau")
    events = spark.table(f"{catalog}.{schema}.silver_cyclone_events")
    premiums = spark.table(f"{catalog}.{schema}.silver_premium_bordereau")
    risk_zones = spark.table(f"{catalog}.{schema}.silver_risk_zone_lookup")

    tables = {
        "gold_claims_summary": build_gold_claims_summary(claims, premiums),
        "gold_loss_ratio_by_risk": build_gold_loss_ratio_by_risk(premiums, claims),
        "gold_event_loss_summary": build_gold_event_loss_summary(claims, events, premiums),
        "gold_portfolio_exposure": build_gold_portfolio_exposure(premiums, risk_zones),
        "gold_claims_development": build_gold_claims_development(claims, premiums),
    }
